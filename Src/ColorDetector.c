...
static void (*ColorReadyFunc)(ColorDetector_Color color);
static ColorDetector_Color color = {.Red = 0, .Green = 0, .Blue = 0};
static bool isColorReady = false;
static bool isColorMeasurment = false;
...
void ColorDetector_Init()
{
  ...
  Task task = {.Func = Loop, .Period = 2000, .Prioryty = 1};
...
static void ColorReady(ColorDetector_Color color)
{
  if(ColorReadyFunc == NULL)
    return;
  ColorReadyFunc(color);
}
